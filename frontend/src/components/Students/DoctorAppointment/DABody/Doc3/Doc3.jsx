...
import doctorImage from '../../Images/Doc3.jpg';
...
export default function Doc1() {
  ...
  const fetchDoctorData = async () => {
    try {
      const response = await axios.get(`http://localhost:7001/api/v1/fetchDoctor/psychologist/${studentId}`, {
        headers: {
          'Authorization': `Bearer ${accessToken}`,
          'Accept': 'application/json'
        }
      });
      sessionStorage.setItem('psychologist', response.data.message[0]._id);
  ...
  const fetchDoctorAvailability = async () => {
    try {
      const psychologist = sessionStorage.getItem('psychologist');
      const response = await axios.get(`http://localhost:7001/api/v1/appointments/doctors/${psychologist}/availability`, {
        headers: {
          'Authorization': `Bearer ${accessToken}`,
        }
      });
