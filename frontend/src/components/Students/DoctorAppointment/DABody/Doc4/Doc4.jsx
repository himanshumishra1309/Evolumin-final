...
import doctorImage from '../../Images/Doc4.jpg';
...
export default function Doc1() {
  ...
  const fetchDoctorData = async () => {
    try {
      const response = await axios.get(`http://localhost:7001/api/v1/fetchDoctor/dermatologist/${studentId}`, {
        headers: {
          'Authorization': `Bearer ${accessToken}`,
          'Accept': 'application/json'
        }
      });
      sessionStorage.setItem('dermatologist', response.data.message[0]._id);
  ...
  const fetchDoctorAvailability = async () => {
    try {
      const dermatologist = sessionStorage.getItem('dermatologist');
      const response = await axios.get(`http://localhost:7001/api/v1/appointments/doctors/${dermatologist}/availability`, {
        headers: {
          'Authorization': `Bearer ${accessToken}`,
        }
      });
