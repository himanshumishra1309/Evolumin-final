...
import doctorImage from '../../Images/Doc6.jpg';
...
export default function Doc1() {
  ...
  const fetchDoctorData = async () => {
    try {
      const response = await axios.get(`http://localhost:7001/api/v1/fetchDoctor/physical-therapist/${studentId}`, {
        headers: {
          'Authorization': `Bearer ${accessToken}`,
          'Accept': 'application/json'
        }
      });
      sessionStorage.setItem('physicalTherapist', response.data.message[0]._id);
  ...
  const fetchDoctorAvailability = async () => {
    try {
      const physicalTherapist = sessionStorage.getItem('physicalTherapist');
      const response = await axios.get(`http://localhost:7001/api/v1/appointments/doctors/${physicalTherapist}/availability`, {
        headers: {
          'Authorization': `Bearer ${accessToken}`,
        }
      });
