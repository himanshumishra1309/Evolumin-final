...
import doctorImage from '../../Images/Doc5.jpg';
...
export default function Doc1() {
  ...
  const fetchDoctorData = async () => {
    try {
      const response = await axios.get(`http://localhost:7001/api/v1/fetchDoctor/allergist/${studentId}`, {
        headers: {
          'Authorization': `Bearer ${accessToken}`,
          'Accept': 'application/json'
        }
      });
      sessionStorage.setItem('allergist', response.data.message[0]._id);
  ...
  const fetchDoctorAvailability = async () => {
    try {
      const allergist = sessionStorage.getItem('allergist');
      const response = await axios.get(`http://localhost:7001/api/v1/appointments/doctors/${allergist}/availability`, {
        headers: {
          'Authorization': `Bearer ${accessToken}`,
        }
      });
